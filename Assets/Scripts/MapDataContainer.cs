...
namespace PipelineDreams {
    ...
    public class MapDataContainer : ScriptableObject {
        ...
        public int GetMapScale(int d) {
            return m.v.GetLength(d);
        }
        ...
        public bool IsLineOfSight(Vector3Int v1, Vector3Int v2) {
            ...
            for (int i = 0; i < m; i++)
                if (GetTile(v2 + v * i, f) == TileAttribute.wall)
                    return false;
            return true;
        ...
        public TileAttribute GetTile(Vector3Int v, int f) {
            if (IsOutofRange(m, v)) return TileAttribute.nothing;
            else
                return m.v[v.x, v.y, v.z].t[f];
        }
        public TileAttribute GetTile(int i, int j, int k, int f) {
            if (IsOutofRange(m, i, j, k)) return TileAttribute.nothing;
            else
                return m.v[i, j, k].t[f];
        }


        public TileAttribute GetTileRelative(Vector3Int v, int f, Entity e) {
            v += e.IdealPosition;
            if (IsOutofRange(m, v)) return TileAttribute.nothing;
            else
                return m.v[v.x, v.y, v.z].t[f];
        }
        public TileAttribute GetTileRelative(int i, int j, int k, int f, Entity e) {
            return GetTileRelative(new Vector3Int(i, j, k), f, e);
        }
        public TileAttribute GetTileRelative(Vector3Int v, int f, Vector3Int EntityPosition) {
            v += EntityPosition;
            if (IsOutofRange(m, v)) return TileAttribute.nothing;
            else
                return m.v[v.x, v.y, v.z].t[f];
        }
        public TileAttribute GetTileRelative(int i, int j, int k, int f, Vector3Int EntityPosition) {
            i += EntityPosition.x;
            j += EntityPosition.y;
            k += EntityPosition.z;
            if (IsOutofRange(m, i, j, k)) return TileAttribute.nothing;
